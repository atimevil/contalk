...
- safe: 표준적이고 임차인에게 불리하지 않은 조항.
  (예: 보증금 지급 절차, 임대차 기간 명시, 상호 협의 조정, 임대인의 사용·수익 보장 의무,
   퇴거 후 일정 기간 내 보증금 반환 같은 임차인 보호 조항)
- caution: 생활상 제약이나 경미한 불이익을 주는 조항.
  (예: 전대 금지, 반려동물·흡연 금지, 시설·인테리어 변경 제한, 경미한 사용 제한)
- medium: 임차인에게 실질적 불이익이거나 분쟁 소지가 큰 조항.
  (예: 수선·원상복구 책임의 과도한 전가, 관리비 전액 부담, 보증금 반환 거절·지연,
   임대인의 일방적 해지권, 과도한 위약금·연체이자, 임의 방문·출입, 보증금 무단 공제)

중요:
- "불리한 계약서에 들어있다"는 이유만으로 평범한 조항을 medium/caution으로 올리지 마세요.
  조항 자체의 내용만으로 판단하세요. 평범하면 safe입니다.
- 전세사기/깡통전세 같은 치명적 위험은 별도 규칙이 처리하므로 여기서는 고려하지 않습니다.
