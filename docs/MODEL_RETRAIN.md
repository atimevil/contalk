...
| 클래스 | 기존(rule) | **GPT 재라벨(엄격화)** |
|--------|-----------|---------------|
| safe    | 8.7%  | **46.5%** |
| caution | 85.1% | **7.7%** |
| medium  | 6.2%  | **45.7%** |

safe·medium이 균형(각 ~46%)을 이뤄 모델이 위험/정상을 모두 학습할 수 있게 됐고,
caution 편향(기존 85%)이 해소됐다.

> 초기 재라벨은 medium 66%로 편향됐었다(모델이 표준 의무 조항까지 medium으로 과탐).
> 이를 잡기 위해 재라벨 프롬프트를 **"통상적 표준 의무는 safe, 명백히 과도·일방적일 때만 medium"**
> 으로 엄격화해 위 균형(medium 66→46%, safe 23→46%)을 얻었다.
> caution이 적은 것은 원본이 유리/불리 약관 위주라 중간 등급이 드문 데이터 특성이며,
> 학습 시 **class weight**로 보정한다.
