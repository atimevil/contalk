...
실행 방법:
    ...
    # GPT 재라벨링 데이터로 학습 (권장 — rule 라벨의 caution 85% 편향을 해소한 데이터)
    python -m backend.ai.train \
        --train-path data/train.gpt.jsonl --val-path data/val.gpt.jsonl \
        --output-dir models/roberta_gpt_v1

데이터 준비:
    1) python -m backend.ai.data_prep            # 원천데이터 → rule 라벨 (train.jsonl)
    2) python scripts/relabel_with_gpt.py --all \\
         --src data/train.jsonl --out data/train.gpt.jsonl   # GPT 재라벨 (권장)

환경변수:
    KLUE_ROBERTA_MODEL_PATH — 완료 후 분류기에서 사용할 저장 경로

요구사항:
    transformers>=4.30, torch>=2.0, scikit-learn, datasets
"""
...
def train(
    ...
    training_args = TrainingArguments(
        ...
        warmup_steps=int(len(train_data) / batch_size * warmup_ratio),
        weight_decay=0.01,
        eval_strategy="epoch",
        save_strategy="epoch",
