...
_CRITICAL_PATTERNS: List[re.Pattern] = [
    ...
    re.compile(r"근저당(?:권)?\s*(?:을|를)?\s*(?:추가(?:로)?\s*)?설정할\s*수\s*있"),
    # 선순위 담보(근저당) 내역 고지 거부
    ...
    # 전입신고/확정일자 취득 자체를 막는 특약 (대항력 원천 차단) — 표현 변형 대응
    re.compile(r"전입신고.{0,25}(?:취득(?:을|를)?\s*)?(?:하지\s*아니|하지\s*않|금지)"),
    re.compile(r"확정일자.{0,20}(?:취득(?:을|를)?\s*)?(?:하지\s*아니|하지\s*않|금지)"),
    # 무권대리 — 소유자 위임/동의 없이 대리인이 체결 (전세사기 전형)
    re.compile(r"위임장\s*없이"),
    re.compile(r"무권\s*대리"),
    re.compile(r"소유자\s*(?:의\s*)?(?:동의|위임)\s*없이.{0,15}(?:대리|체결)"),
    # 이중계약 — 동일 주택을 중복 임대 (보증금 사기 위험)
    re.compile(r"이중\s*(?:으로\s*)?임대"),
    re.compile(r"중복\s*(?:으로\s*)?(?:임대|임대차)"),
    re.compile(r"별도의?\s*임대차\s*계약을\s*체결할\s*수\s*있"),
]
...
_RISK_RULES: List[Tuple[re.Pattern, str]] = [
    # ── 재현율 보강: 표준에서 벗어난 불리 특약 ───────────────────────────────
    # 2기 미만(1회/즉시) 연체로 해지·명도 — 주택임대차보호법 위반 소지
    (re.compile(r"(?:1\s*회|한\s*번|일\s*회).{0,20}연체.{0,40}(?:즉시|최고\s*(?:절차\s*)?없이).{0,15}(?:해지|해제|명도)"), "medium"),
    (re.compile(r"최고\s*(?:절차\s*)?없이.{0,25}(?:해지|해제|명도)"), "medium"),
    # 보증금 반환 지연/거절 면책 (이자·손해배상 청구 불가) — 임차인 불리
    (re.compile(r"반환.{0,25}지연.{0,50}(?:이자|손해배상).{0,20}청구하지\s*(?:아니|않)"), "medium"),
    # 임차인 동의 없이 임대인 임의 매도
    (re.compile(r"동의\s*없이.{0,25}(?:제3자|타인|매수인).{0,15}(?:매도|매각)"), "medium"),
    (re.compile(r"동의\s*없이.{0,20}매도할\s*수\s*있"), "medium"),
    # 수선 책임 광범위 전가 ("모든/일체 수선 ... 임차인 부담")
    (re.compile(r"수선.{0,30}임차인\s*(?:의|이|에게)?\s*부담"), "medium"),
    # 중도해지 위약금 (표현 변형 — "해지를 할 수 없으며 ... 위약금")
    (re.compile(r"중도\s*해지.{0,45}위약금"), "medium"),
    # 별도 부담금 임차인 전가 / 일방적 차임 인상
    (re.compile(r"별도\s*부담금.{0,20}임차인.{0,10}납부"), "medium"),
    (re.compile(r"(?:사전\s*)?(?:통지|통보)\s*없이.{0,20}차임.{0,10}인상"), "medium"),
    # 관리비 폭탄 — 임대인이 임의/일방적으로 인상·변동·책정
    (re.compile(r"관리비.{0,20}(?:임의|일방적|정하는\s*바).{0,12}(?:인상|변동|부과|책정)"), "medium"),
    ...
    # ── 주의 ────────────────────────────────────────────────────────────────
    # 전대 금지 (조사 허용: 전대를/전대을 금지)
    (re.compile(r"전대(?:를|을)?\s*(?:금지|불가|할\s*수\s*없)"), "caution"),
    # 반려동물 금지
    (re.compile(r"반려\s*동물|애완\s*동물"), "caution"),
    # 흡연 금지 (조사 허용: 흡연을 금지)
    (re.compile(r"흡연(?:을|를)?\s*(?:금지|불가)"), "caution"),
    # 인테리어/시설 변경 금지
...
# ---------------------------------------------------------------------------
# 표준계약서 안전 베이스라인 — 국토부 주택임대차표준계약서의 정형·보호 조항
# (제1~제10조의 표준 문구). 이 패턴에 매치되면 모델/일반 규칙보다 우선하여 safe.
# 표준에서 "벗어난" 불리 특약은 이 패턴에 매치되지 않으므로 위험 분류가 유지된다.
# 치명 위험(_CRITICAL_PATTERNS)보다는 후순위다.
# ---------------------------------------------------------------------------

_STANDARD_SAFE_PATTERNS: List[re.Pattern] = [
    # 제3조: 입주 전 수리 — 시설물/비용부담 별도 합의 (표준 안내 조항)
    re.compile(r"수리가?\s*필요한\s*시설물.{0,25}별도\s*합의"),
    # 제4조 ①: 임차인은 임대인 동의 없이 전대·양도 불가 (표준 제한)
    re.compile(r"임차인은\s*임대인\s*(?:의\s*)?동의\s*없이.{0,45}(?:할\s*수\s*없|양도)"),
    # 제4조 ③: 주요설비 노후 수선은 임대인 부담 (표준·균형 배분)
    re.compile(r"주요설비.{0,25}노후.{0,15}수선은\s*임대인"),
    # 제6조: 채무불이행과 손해배상 (표준 문구)
    re.compile(r"채무를?\s*이행하지\s*아니하는\s*때"),
    # 제7조: 2기 차임 연체 시 해지 (주임법 기준 — 표준)
    re.compile(r"2\s*기의?\s*차임액에\s*달하도록\s*연체"),
    # 제8조: 계약갱신요구권 행사 기간 안내 (표준)
    re.compile(r"임대차기간이?\s*끝나기\s*6개월\s*전부터\s*2개월\s*전까지"),
    # 제9조: 노후·통상 파손은 원상복구의무에서 제외 (임차인 보호)
    re.compile(r"원상복구(?:의무)?에\s*포함되지\s*(?:아니|않)"),
    # 단순 확인 문구 ("임차인은 위/상기 사항을 확인하였다") — 의무 없는 확인일 뿐 위험 아님
    re.compile(r"(?:위|상기)\s*(?:사항|내용)(?:을|를)?\s*(?:모두\s*)?확인하였"),
]


def _is_standard_safe(text: str) -> bool:
    """표준계약서의 정형·보호 조항 문구인지 여부."""
    if not text:
        return False
    normalized = re.sub(r"\s+", " ", text).strip()
    return any(p.search(normalized) for p in _STANDARD_SAFE_PATTERNS)
...
_DOWNGRADE_TO_CAUTION: List[re.Pattern] = [
    ...
    # 전대 금지(임대인 동의 필요)는 표준적 생활 제약 → 모델이 medium으로 봐도 주의로 낮춤
    re.compile(r"전대(?:를|을)?\s*(?:금지|불가)"),
]
...
def classify_risk(clauses: List[dict]) -> List[dict]:
    ...
    for clause in clauses:
        ...
        if _is_critical(text):
            ...
            risk = "high"
        # 0.5 표준계약서 정형·보호 조항 — 모델/규칙 오탐 방지 위해 safe 우선
        elif _is_standard_safe(text):
            logger.debug("표준 안전 베이스라인 safe | %.50s...", text)
            risk = "safe"
